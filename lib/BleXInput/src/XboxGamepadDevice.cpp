...
static inline T clampValue(T v, U lo, V hi) {
...
{    
    ...
    ESP_LOGD(LOG_TAG, "XboxGamepadCallbacks::onWrite, Size: %d, DC enable: %d, magnitudeWeak: %d, magnitudeStrong: %d", 
...
void XboxGamepadDevice::setLeftThumb(int16_t x, int16_t y) {
    x = clampValue(x, XBOX_STICK_MIN, XBOX_STICK_MAX);
    y = clampValue(y, XBOX_STICK_MIN, XBOX_STICK_MAX);
...
void XboxGamepadDevice::setRightThumb(int16_t z, int16_t rZ) {
    z = clampValue(z, XBOX_STICK_MIN, XBOX_STICK_MAX);
    rZ = clampValue(rZ, XBOX_STICK_MIN, XBOX_STICK_MAX);
...
void XboxGamepadDevice::setLeftTrigger(uint16_t value) {
    value = clampValue(value, XBOX_TRIGGER_MIN, XBOX_TRIGGER_MAX);
...
void XboxGamepadDevice::setTriggers(uint16_t left, uint16_t right) {
    left = clampValue(left, XBOX_TRIGGER_MIN, XBOX_TRIGGER_MAX);
    right = clampValue(right, XBOX_TRIGGER_MIN, XBOX_TRIGGER_MAX);
...
void XboxGamepadDevice::sendGamepadReport(bool defer) {
    ...
    if (_lastInputTsUs > 0) {
        int64_t delta = now - _lastInputTsUs;
        ESP_LOGI(LOG_TAG, "Latency: %lld us from input->notify", (long long)delta);
    }
    _input->notify();
}